...
export default function CategoriesDisplay({ party, inParty, fetchData }) {
  ...
  const parameters = [
    ["Experience", party.party.searchParameters.experience],
    ["Type", party.party.searchParameters.type],
    ["Genre", party.party.searchParameters.genre],
    ["Level", party.party.searchParameters.level],
  ...
  return (
    <div className="categories-display">
      ...
      {inParty === "dm" ? (
        <div className="open-modify-party" onClick={() => modifyParams()}>
          ⚙️
        </div>
      ) : (
        ""
      ...
      <div className="categories-list">
        {parameters.map((item) => {
          return (
            <div key={item} className={classNames({ category: true })}>
              {item[0]}: {item[1]}
            </div>
          );
        })}
