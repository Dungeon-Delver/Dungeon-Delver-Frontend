...
export default function FindParties() {
    ...
    try {
      ...
      if (data.data.response.parties.length > 0) {
        ...
        if (!data.data.response.reachedEnd) {
          handlePrefetch(
            null,
            data.data.response.parties[data.data.response.parties.length - 1]
          );
          setFetchedPrev(true);
  ...
  const handleSearchPartyByName = async () => {
    ...
    setNext([]);
    try {
      const data = await axios.post(`${BACKEND_URL}party/search`, {
        user: user.id,
        name: partyName,
      });
      if (data.data.response.parties.length > 0) {
  ...
  const handleNext = async () => {
    setQueueNext(false);
    setQueuePrev(false);
    if (next.length > 0 && next[0].objectId === searchResults[0].objectId) {
      setNext([]);
      setNextDisabled(0);
      return;
    }
    if (searchMode === "params") {
      ...
      if (next.length > 0 && fetchedNext && fetchedPrev) {
        setSearchResults(next);
        if (nextDisabled > 1) {
          handleNamePrefetch(next[0], next[next.length - 1]);
        } else {
          handleNamePrefetch(next[0], null);
          setNextDisabled(0);
          setFetchedNext(true);
        }
        setNext([]);
  ...
  const handlePrevious = async () => {
    setQueueNext(false);
    setQueuePrev(false);
    if (prev.length > 0 && prev[0].objectId === searchResults[0].objectId) {
      setPrev([]);
      setPrevDisabled(0);
      return;
    }
    if (searchMode === "params") {
      ...
      if (prev.length > 0 && fetchedPrev && fetchedNext) {
        setSearchResults(prev);
        if (prevDisabled > 1) {
          handleNamePrefetch(prev[0], prev[prev.length - 1]);
        } else {
          handleNamePrefetch(null, prev[prev.length - 1]);
          setFetchedNext(true);
          setPrevDisabled(0);
        }
        setNext([]);
        ...
        handleNamePrefetch(searchResults[0], null);
      }
      setNextDisabled(2);
    }
  };
  ...
  const handleNamePrefetch = async (first, last) => {
    if (first !== null) {
      //prefetch previous
      setFetchedPrev(false);
      const JSON_OBJECT = {
  ...
  if (loadingParty) {
    ...
    bottom = (
      <h2 className="no-parties">
        There are no parties that meet these search criteria
      </h2>
    );
  } else if (partyFailed) {
    bottom = (
    ...
    bottom = (
      <>
        ...
        <div className="button-20-container">
          <button
            disabled={prevDisabled === 0}
            onClick={
              fetchedPrev && fetchedNext
                ? () => handlePrevious()
                : () => {
                    setQueuePrev(true);
                    setQueueNext(false);
            ...
            className="previous-button button-20"
          >
            Previous
          </button>
          <div className="page-number">Page {page}</div>
          <button
            disabled={nextDisabled === 0}
            onClick={
              fetchedNext && fetchedPrev
                ? () => handleNext()
                : () => {
                    setQueueNext(true);
                    setQueuePrev(false);
            ...
            className="next-button button-20"
          >
            Next
          </button>
        </div>
    ...
    <div className="find-parties">
      ...
      <form className="find-parties-by-name-form">
        ...
        <button
          ...
          onClick={(event) => handleSubmitByName(event)}
        >
          <div className="button__text">Find Dungeons</div>
        </button>
      </form>
