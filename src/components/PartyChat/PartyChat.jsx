    ...
    try {
      ...
      if (firstMessage != null) {
        response = await axios.post(
          `${BACKEND_URL}party/${partyId}/messages/`,
          { firstMessage: firstMessage, userId: user.id }
        );
      ...
      if (lastMessage.senderId === user.id) {
        for (let i = 0; i < pendingMessages.length; i++) {
          if (pendingMessages[i].messageId === lastMessage.messageId) {
            ...
            setPendingMessages([
              ...pendingMessages.slice(0, i),
              ...pendingMessages.slice(i + 1),
            ]);
            break;
          }
        }
    ...
    if (newMessage !== "") {
      try {
        const response = await axios.get(
          `${BACKEND_URL}user/${user.id}/in/party/${partyId}`
        );
  ...
  return (
    ...
    >
      <ol className="messages-list">
        <button
          className={"load-more-messages button-81"}
          disabled={reachedTop}
          onClick={() => loadMore(messages[0])}
        >
          Load More
        </button>
        {messages.map((message, i) => {
          return (
            <ChatMessage
              key={i}
              message={message}
              prevMessage={i === 0 ? true : messages[i - 1]}
      ...
      {inParty === "dm" || inParty === "player" ? (
        <>
          ...
          <button
            ...
            className="send-message-button button-46"
          >
            Send
          </button>
        </>
          ...
          <>
            <Link to={`/user/${message.senderId}`}>
              <div className="chat-img-container">
                <img src={message.user.picture} alt={message.user.username} />
              </div>
            </Link>
            <div className="chat-user">{message.user.username}</div>
          </>
        ) : (
          ""
        )}
